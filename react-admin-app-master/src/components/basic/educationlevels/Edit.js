...
const EditEduLevel=()=>{
    ...
    const handle_submit=()=>{
        ...
        if(result.valid===true){
            ...
            API.put('/api/education/'+id+'/',JSON.stringify({name:name,updatedby:profile.role})).then(response=>{
    ...
    return(
        ...
        <CRow>
            ...
            <CCard>
                <CCardHeader>
                <h3>{t('translations.Basic_Data.Education_Levels.Edit.header')}</h3>
                </CCardHeader>
                <CCardBody>
                    <CForm>
                        <CFormGroup>
                            <CLabel htmlFor="selectCode">{t('translations.Basic_Data.Education_Levels.table_headers.name')} *</CLabel>
                            <CInput onChange={(event)=>setName(event.target.value)} value={name} type="text" id="selectCode" name="selectCode" placeholder="Enter education level type name.."/>
                        ...
                        <CFormGroup>
                            ...
                            <CTextarea onChange={(event)=>setDescription(event.target.value)} value={description} type="text" id="name" name="name"/>
                        </CFormGroup>
                    </CForm>
                </CCardBody>
                <CCardFooter>
